=== Solid Construction ===
Contributors: catchthemes
Tags: two-columns, three-columns, left-sidebar, right-sidebar, grid-layout, flexible-header, custom-background, custom-colors, custom-header, custom-menu, custom-logo, editor-style, featured-image-header, featured-images, footer-widgets, full-width-template, rtl-language-support, sticky-post, theme-options, threaded-comments, translation-ready, blog, photography, e-commerce, portfolio
Requires at least: 5.1
Tested up to: 5.6
Requires PHP: 5.6
Stable tag: 2.2.1
License: GNU General Public License v3 or later
License URI: http://www.gnu.org/licenses/gpl-3.0.html

Solid Construction is a free construction WordPress theme that is highly customizable and fits perfectly for construction companies to put their website in the forefront.

== Description ==

Solid Construction is a Free Construction WordPress theme, which is highly customizable and fits perfectly for construction companies to put their website in the forefront. Solid Construction has a capable framework for crafting impressive, modern websites for the architectural firms, building companies, and freelance architects. Your website will look powerful yet elegant on mobile devices and desktop computers, regardless of screen resolution with its responsive design. With Solid Construction, you are able to build an astounding web page with excellent loading speeds resulting in a superior customer experience. Solid Construction is easy-to-use, feature-rich and visually stunning. The theme is extremely creative, modern, and user-friendly. With all the amazing features included, Solid Construction will take care of everything to give your construction website an awesome online presence which in turn will definitely attract more customers to your business. To top it all, the theme is translation ready. For more details, check out the Theme Instructions at https://catchthemes.com/themes/solid-construction/#theme-instructions, support at https://catchthemes.com/support/ and Demo at https://catchthemes.com/demo/solid-construction

For more information about Solid Construction please go to https://catchthemes.com/themes/solid-construction/


== Installation ==

1. In your admin panel, go to Appearance -> Themes and click the 'Add New' button.
2. Type in Solid Construction in the search form and press the 'Enter' key on your keyboard.
3. Click on the 'Activate' button to use your new theme right away.
4. Go to https://catchthemes.com/themes/solid-construction/#theme-instructions for a guide on how to customize this theme.
5. Navigate to Appearance -> Customize in your admin panel and customize to taste.

== Frequently Asked Questions ==

= Does this theme support any plugins? =

Theme supports all plugins from catchplugins.com and WooCommerce Plugin.

= Where can I find theme documentation? =

You can check our Theme Instructions at https://catchthemes.com/themes/solid-construction in the Theme Instructions Tab.

= Where can I find theme demo? =

You can check our Theme Demo at https://catchthemes.com/demo/solid-construction/

= More FAQ =

For more FAQs, visit https://catchthemes.com/frequently-asked-questions/

== Changelog ==

= 2.2.1 (Released: January 08, 2021) =
* Bug Fixed: Header Video issue
* Bug Fixed: Multi flex-height, changed to flex-height and flex-width

= 2.2 (Released: August 11, 2020) =
* Removed: Unnecessary file (jquery.cycle2.suffle.min.js)
* Updated: WP 5.5 compatibility
* Updated: All images
* Updated: Screenshot

= 2.1 (Released: May 22, 2020) =
* Bug Fixed: content.php headline condition
* Updated: readme.txt and style.css as per the new requirement

= 2.0.7 (Released: May 11, 2020) =
* Updated: CSS
* Added: Icon for telephone number in Social Menu

= 2.0.6 (Released: April 08, 2020) =
* Added: Id's for section main div

= 2.0.5 (Released: February 24, 2020) =
* Bug Fixed: HTML issue in searchform.php

= 2.0.4 (Released: February 03, 2020) =
* Bug Fixed: Breadcrumb issue with disabled on homepage

= 2.0.3 (Released: January 15, 2020) =
* Bug Fixed: h1 tag issue
* Bug Fixed: Breadcrumb issue when there are no categories

= 2.0.2 (Released: November 15, 2019) =
* Bug Fixed: Multi Category Sanitization issue

= 2.0.1 (Released: August 08, 2019) =
* Added: Support for wp_body_open action
* Bug Fixed: Added function_exists check for get_the_privacy_policy_link function usage

= 2.0 (Released: December 19, 2018 ) =
* Added: Theme support for block styles
* Added: Theme support for full and wide align images
* Added: Theme support for responsive embeds
* Added: Theme support for custom editor font sizes
* Added: Theme support for custom color scheme
* Added: Theme support for for editor styles
* Updated: Metabox UI
* Updated: Readme file as per the new requirement

= 1.1.4 (Released: October 24, 2018 ) =
* Bug Fixed: Testimonial Typo corrected

= 1.1.3 (Released: August 21, 2018 ) =
* Bug Fixed: Site Overlay issue
* Updated: Page max-width changed 2560px
* Updated Content delivered through the_content filters

= 1.1.2 (Released: August 07, 2018 ) =
* Bug Fixed: Header Text Color option not working for tagline

= 1.1.1 (Released: August 01, 2018 ) =
* Bug Fixed: Improper escaping for aria-label on menus
* Updated: Breadcrumb functions - used the_archive_title() instead of single_cat_title and single_tag_title

= 1.1 (Released: July 31, 2018 ) =
* Added: Child theme ready functions

= 1.0 (Released: July 06, 2018) =
* Added: Social Menu in primary menu area
* Bug Fixed: Layout issue on header menu with search
* Bug Fixed: Menu Flickering on site load
* Updated: Portfolio image size changed to 470 to fix mobile design

= 0.1 (Released: July 04 2018) =
* Initial release

== Copyright ==

Solid Construction WordPress Theme, Copyright 2018 Catchthemes.com
Solid Construction is distributed under the terms of the GNU General Public License v2

== Resources ==

* Based on Underscores http://underscores.me/, (C) 2012-2018 Automattic, Inc., [GPLv2 or later](https://www.gnu.org/licenses/gpl-2.0.html)

* JS Files
	Fitvids 1.1, Copyright 2013, Chris Coyier
	License: the WTFPL license - http://sam.zoy.org/wtfpl/
    http://fitvidsjs.com/

	HTML5 Shiv v3.7.0, Copyright 2014 Alexander Farkas
	License: MIT/GPL2
	Source: https://github.com/aFarkas/html5shiv

	jQuery Cycle 2, Copyright (c) 2014 M. Alsup
	License: MIT/GPL
    http://jquery.malsup.com/cycle2/

    jquery-match-height (C) 2014 @liabru
    License: MIT
    http://brm.io/jquery-match-height/

* Images
	All Images are licensed under Creative Commons Zero (CC0) license

	Header image
		https://stocksnap.io/photo/construction-builder-A5Q1BUP3OF

	Screenshot Image
		https://stocksnap.io/photo/people-man-T78VYSKHFS

* Icons
	Font Awesome icons, Copyright Dave Gandy
	License: SIL Open Font License, version 1.1.
	Source: http://fontawesome.io/

All other images are self shot image by (CatchThemes.com) and released under same license as theme
